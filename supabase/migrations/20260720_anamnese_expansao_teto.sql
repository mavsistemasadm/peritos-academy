-- Regra de expansão: se a rota fechar curta demais (< meses_minimo_antes_expandir,
-- default 6) no ritmo declarado, puxa as próximas trilhas por ordem de votos
-- (respeitando teto_expandido, default 7) até aproximar a meta de 12 meses
-- SEM ultrapassá-la. Reaproveita anamnese_calcular_cronograma() como única
-- fonte de verdade da matemática de carga/semanas — cada trilha candidata é
-- inserida, o cronograma inteiro é recalculado, e só fica se não estourar a
-- meta; senão a inserção é desfeita e a expansão para ali.

alter table public.config_anamnese add column if not exists teto_expandido int not null default 7;
alter table public.config_anamnese add column if not exists meses_minimo_antes_expandir numeric not null default 6;
...
begin
  ...
  v_ordem := 1;
  -- Âncora: FOR sempre entra primeiro quando Q1 indica quem nunca atuou ou
  -- só estudou (Fase C.3 da curadoria).
  ...
  if v_meses_atual < v_config.meses_minimo_antes_expandir then
    ...
    loop
      ...
      insert into public.plano_trilhas (plano_id, usuario_id, trilha_id, ordem, votos, forcada_regra_iniciante)
      values (v_plano_id, v_usuario, v_candidato.trilha_id, v_qtd_trilhas_atual + 1, v_candidato.votos, false);
      ...
      if v_meses_tentativo > v_config.meta_meses then
        -- ultrapassaria a meta: desfaz essa trilha e para a expansão aqui.
  ...
  return jsonb_build_object('ok', true, 'plano_id', v_plano_id, 'avatar', v_avatar, 'regra_iniciante_aplicada', v_regra_iniciante, 'expansao_aplicada', v_expandiu);
end;
$function$;
